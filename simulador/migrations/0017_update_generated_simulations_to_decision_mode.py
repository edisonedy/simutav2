...
def forwards(apps, schema_editor):
    ...
    simulaciones = (
        Simulacion.objects
        .filter(plantilla_origen__codigo='global-decision-3rondas-v1')
        .select_related('materia_malla__materia', 'materia_malla__nivel', 'perfil_materia_ia', 'plantilla_origen')
    )
    for simulacion in simulaciones:
        rondas_parametros = []
        for ronda in simulacion.plantilla_origen.rondas.filter(activo=True).order_by('numero'):
        ...
        if simulacion.titulo.startswith('Examen SimutaV2 - '):
        ...
        simulacion.save(update_fields=[
            'titulo', 'situacion_inicial', 'parametros', 'version_configuracion',
        ])


def backwards(apps, schema_editor):
    Simulacion = apps.get_model('simulador', 'Simulacion')
    for simulacion in Simulacion.objects.filter(plantilla_origen__codigo='global-decision-3rondas-v1'):
        parametros = dict(simulacion.parametros or {})
        parametros.pop('modo', None)
        parametros.pop('rondas', None)
        simulacion.parametros = parametros
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('simulador', '0016_seed_decision_options'),
    ]
